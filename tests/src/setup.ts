...
import { MembraneInvitationsClient } from '../../ui/src/membrane-invitations-client.js';
import { MembraneInvitationsStore } from '../../ui/src/membrane-invitations-store.js';

const testHappUrl = dirname(fileURLToPath(import.meta.url)) + '/../../workdir/membrane-invitations_test.happ';
...
async function addPlayer(scenario: Scenario) {
  ...
  const store = new MembraneInvitationsStore(
  	new MembraneInvitationsClient(player.appWs as any, 'membrane_invitations_test'),
  );
  return {
  	store,
  	player,
