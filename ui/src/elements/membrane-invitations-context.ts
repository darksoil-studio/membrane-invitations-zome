...
export class MembraneInvitationsContext extends LitElement {
  ...
  connectedCallback() {
    ...
    if (!this.role) {
      throw new Error(`<membrane-invitations-context> must have a role="YOUR_DNA_ROLE" property, eg: <membrane-invitations-context role="role1">`);
    }
    if (!this.client) {
    ...
    this.store = new MembraneInvitationsStore(new MembraneInvitationsClient(this.client, this.role, this.zome));
  }
